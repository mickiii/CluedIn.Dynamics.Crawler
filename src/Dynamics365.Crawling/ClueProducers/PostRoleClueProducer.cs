...
{
    public abstract class PostRoleClueProducer<T> : DynamicsClueProducer<T> where T : PostRole
    {
        private readonly IClueFactory _factory;

        private readonly Dynamics365CrawlJobData _dynamics365CrawlJobData;

        public PostRoleClueProducer([NotNull] IClueFactory factory, IAgentJobProcessorState<CrawlJobData> state)
        {
            if (factory == null)
        ...
        protected override Clue MakeClueImpl([NotNull] T input, Guid accountId)
        {
            ...
            var clue = this._factory.Create(EntityType.Unknown, input.PostRoleId.ToString(), accountId);

            var data = clue.Data.EntityData;

            if (Uri.TryCreate(string.Format("{0}/main.aspx?pagetype=entityrecord&etn=postrole&id={1}", _dynamics365CrawlJobData.Api, input.PostRoleId.ToString()), UriKind.Absolute, out Uri uri))
                data.Uri = uri;
            ...
            */
            data.Properties[Dynamics365Vocabulary.PostRole.OrganizationId] = input.OrganizationId.PrintIfAvailable();
            data.Properties[Dynamics365Vocabulary.PostRole.OrganizationIdName] = input.OrganizationIdName.PrintIfAvailable();
            data.Properties[Dynamics365Vocabulary.PostRole.PostId] = input.PostId.PrintIfAvailable();
            data.Properties[Dynamics365Vocabulary.PostRole.PostIdName] = input.PostIdName.PrintIfAvailable();
            data.Properties[Dynamics365Vocabulary.PostRole.PostRoleId] = input.PostRoleId.PrintIfAvailable();
            data.Properties[Dynamics365Vocabulary.PostRole.RegardingObjectId] = input.RegardingObjectId.PrintIfAvailable();
            data.Properties[Dynamics365Vocabulary.PostRole.RegardingObjectTypeCode] = input.RegardingObjectTypeCode.PrintIfAvailable();
            data.Properties[Dynamics365Vocabulary.PostRole.Type] = input.Type.PrintIfAvailable();
            data.Properties[Dynamics365Vocabulary.PostRole.TypeName] = input.TypeName.PrintIfAvailable();
