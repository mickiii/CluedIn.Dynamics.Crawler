...
{
    public abstract class BulkDeleteFailureClueProducer<T> : DynamicsClueProducer<T> where T : BulkDeleteFailure
    {
        private readonly IClueFactory _factory;

        private readonly Dynamics365CrawlJobData _dynamics365CrawlJobData;

        public BulkDeleteFailureClueProducer([NotNull] IClueFactory factory, IAgentJobProcessorState<CrawlJobData> state)
        {
            if (factory == null)
        ...
        protected override Clue MakeClueImpl([NotNull] T input, Guid accountId)
        {
            ...
            var clue = this._factory.Create(EntityType.Unknown, input.BulkDeleteFailureId.ToString(), accountId);

            var data = clue.Data.EntityData;

            if (Uri.TryCreate(string.Format("{0}/main.aspx?pagetype=entityrecord&etn=bulkdeletefailure&id={1}", _dynamics365CrawlJobData.Api, input.BulkDeleteFailureId.ToString()), UriKind.Absolute, out Uri uri))
                data.Uri = uri;
            ...
            /*
             if (input.RegardingObjectId != null)
                         ...
                         if (input.BulkDeleteOperationId != null)
                            _factory.CreateOutgoingEntityReference(clue, EntityType.bulkdeleteoperation, EntityEdgeType.Parent, input, input.BulkDeleteOperationId);
            ...
            */
            data.Properties[Dynamics365Vocabulary.BulkDeleteFailure.ErrorDescription] = input.ErrorDescription.PrintIfAvailable();
            data.Properties[Dynamics365Vocabulary.BulkDeleteFailure.AsyncOperationId] = input.AsyncOperationId.PrintIfAvailable();
            data.Properties[Dynamics365Vocabulary.BulkDeleteFailure.BulkDeleteFailureId] = input.BulkDeleteFailureId.PrintIfAvailable();
            data.Properties[Dynamics365Vocabulary.BulkDeleteFailure.OwningBusinessUnit] = input.OwningBusinessUnit.PrintIfAvailable();
            data.Properties[Dynamics365Vocabulary.BulkDeleteFailure.RegardingObjectId] = input.RegardingObjectId.PrintIfAvailable();
            data.Properties[Dynamics365Vocabulary.BulkDeleteFailure.ErrorNumber] = input.ErrorNumber.PrintIfAvailable();
            data.Properties[Dynamics365Vocabulary.BulkDeleteFailure.OrderedQueryIndex] = input.OrderedQueryIndex.PrintIfAvailable();
            data.Properties[Dynamics365Vocabulary.BulkDeleteFailure.BulkDeleteOperationId] = input.BulkDeleteOperationId.PrintIfAvailable();
            data.Properties[Dynamics365Vocabulary.BulkDeleteFailure.OwningUser] = input.OwningUser.PrintIfAvailable();
            data.Properties[Dynamics365Vocabulary.BulkDeleteFailure.RegardingObjectIdYomiName] = input.RegardingObjectIdYomiName.PrintIfAvailable();
            data.Properties[Dynamics365Vocabulary.BulkDeleteFailure.RegardingObjectTypeCode] = input.RegardingObjectTypeCode.PrintIfAvailable();
            data.Properties[Dynamics365Vocabulary.BulkDeleteFailure.RegardingObjectIdName] = input.RegardingObjectIdName.PrintIfAvailable();
            data.Properties[Dynamics365Vocabulary.BulkDeleteFailure.OwnerId] = input.OwnerId.PrintIfAvailable();
            data.Properties[Dynamics365Vocabulary.BulkDeleteFailure.OwnerIdType] = input.OwnerIdType.PrintIfAvailable();
