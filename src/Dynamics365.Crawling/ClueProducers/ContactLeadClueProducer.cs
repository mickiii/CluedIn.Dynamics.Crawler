...
namespace CluedIn.Crawling.Dynamics365.ClueProducers
{
    public abstract class ContactLeadClueProducer<T> : DynamicsClueProducer<T> where T : ContactLead
    {
        ...
        protected override Clue MakeClueImpl([NotNull] T input, Guid accountId)
        {
            ...
            if (Uri.TryCreate(string.Format("{0}/main.aspx?pagetype=entityrecord&etn=contactlead&id={1}", _dynamics365CrawlJobData.Api, input.ContactLeadId.ToString()), UriKind.Absolute, out Uri uri))
                data.Uri = uri;
