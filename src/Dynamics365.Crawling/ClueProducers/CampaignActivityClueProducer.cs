...
namespace CluedIn.Crawling.Dynamics365.ClueProducers
{
    public abstract class CampaignActivityClueProducer<T> : DynamicsClueProducer<T> where T : CampaignActivity
    {
        ...
        protected override Clue MakeClueImpl([NotNull] T input, Guid accountId)
        {
            ...
            if (Uri.TryCreate(string.Format("{0}/main.aspx?pagetype=entityrecord&etn=campaignactivity&id={1}", this._dynamics365CrawlJobData.Url, input.ActivityId.ToString()), UriKind.Absolute, out Uri uri))
                data.Uri = uri;
