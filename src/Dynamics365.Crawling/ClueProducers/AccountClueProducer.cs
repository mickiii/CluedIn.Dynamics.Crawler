...
namespace CluedIn.Crawling.Dynamics365.ClueProducers
{
    public abstract class AccountClueProducer<T> : DynamicsClueProducer<T> where T : Account
    {
        ...
        protected override Clue MakeClueImpl([NotNull] T input, Guid accountId)
        {
            ...
            if (Uri.TryCreate(string.Format("{0}/main.aspx?pagetype=entityrecord&etn=account&id={1}", _dynamics365CrawlJobData.Api, input.AccountId.ToString()), UriKind.Absolute, out Uri uri))
                data.Uri = uri;
